...
class gluster::vardir {	# module vardir snippet
	...
	if "${::puppet_vardirtmp}" == '' {
		...
		file { "${tmp}":
			...
			owner => "${::gluster::params::misc_owner_vardir_tmp_dir}",
			group => "${::gluster::params::misc_group_vardir_tmp_dir}",
			mode => 600,
	...
	file { "${module_vardir}":		# /var/lib/puppet/tmp/gluster/
		...
		purge => true,			# purge all unmanaged files
		force => true,			# also purge subdirs and links
		owner => "${::gluster::params::misc_owner_vardir_gluster_tmp_dir}",
		group => "${::gluster::params::misc_group_vardir_gluster_tmp_dir}",
		mode => 600,
		backup => false,
